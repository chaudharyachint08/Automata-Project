...
{//Finds the Kleene * Closure on basis of previous machine
	...
	if(utoms==NULL)
	{ puts("Hey! INVALID R.E. due to more '*' operators"); getch(); return; }
	...
	for(k=0;k<=count;k++)
      {if(r[k]==1) 
	   {printf("   [C%d]",ind);
		...
		{
			printf("* ");
			for(i=0;i<(radix-rad(ind));i++){printf(" ");}
			printf(": | ");
		}
	    else
		{
